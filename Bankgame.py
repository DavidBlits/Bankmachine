...
while True:
    action=input(f"Enter number, {BLUE}1:Display Balance{RESET}, {YELLOW}2:Withdraw{RESET}, {CYAN}3:Deposit{RESET}, {RED}4:Exit{RESET}")
    #show balance
    if action=='1':
    ...
    #Withdraw assuming the fee applies for the up to -500 rule
        ...
        if balance-num>0:
        ...
        elif 0>((balance-num)+(balance-num)*0.2):
            if -500<((balance-num)+(num*0.2)):
            ...
            elif choice=="Yes" or choice=='yes' or choice=='y' or choice=='Y':
